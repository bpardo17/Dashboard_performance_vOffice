export function initializeMapReservasTotales() {
  ...
  am5.ready(function () {
    ...
    maybeDisposeRoot("chartdiv__visitas-web");
    maybeDisposeRoot("chartdiv__visitas-app");
    maybeDisposeRoot("chartdiv__reservas");

    // Create root element
    var root = am5.Root.new("chartdiv__reservas_totales");
    root.setThemes([
    ...
    try{
      ...
      for (var i = 0; i < secureDataRiu.reservas_totales.map.length; i++) {
        var pointMap = secureDataRiu.reservas_totales.map[i];
        addCity(pointMap.long, pointMap.lat, pointMap.value, pointMap.color, i);
      }
      ...
      function addCity(longitude, latitude, value, color) {
        ...
        }, i * 180); // Delay of 100ms for each element
      }
